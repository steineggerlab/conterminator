...
std::vector<struct Command> commands = {
        ...
        {"distanceton",      distanceton,      &par.distanceton,          COMMAND_HIDDEN,
                ...
                {{"queryDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::taxSequenceDb },
                 ...
                 {"alnDB",   DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"createstats",          createstats,          &par.threadsandcompression,         COMMAND_TAXONOMY,
        ...
        {"multipletaxas",          multipletaxas,          &par.onlythreads,         COMMAND_HIDDEN,
        ...
        {"easy-search",          easysearch,           &par.easysearchworkflow,   COMMAND_HIDDEN,
                ...
                CITATION_SERVER | CITATION_MMSEQS2,{{"queryFastaFile[.gz]",  DbType::ACCESS_MODE_INPUT,  DbType::NEED_DATA,  &DbValidator::flatfile },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"easy-linsearch",       easylinsearch,        &par.easylinsearchworkflow,COMMAND_HIDDEN,
                ...
                "Milot Mirdita <milot@mirdita.de> & Martin Steinegger <martin.steinegger@mpibpc.mpg.de>",
                "<i:queryFastaFile[.gz]> <i:targetFastaFile[.gz]>|<i:targetDB> <o:alignmentFile> <tmpDir>",
                CITATION_MMSEQS2|CITATION_LINCLUST, {{"queryFastaFile[.gz]",  DbType::ACCESS_MODE_INPUT,  DbType::NEED_DATA,  &DbValidator::flatfile },
                                          {"targetDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                          {"alignmentFile", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile  },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"easy-linclust",        easylinclust,         &par.easylinclustworkflow, COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2|CITATION_LINCLUST, {{"queryFastaFile[.gz]",  DbType::ACCESS_MODE_INPUT,  DbType::NEED_DATA,  &DbValidator::flatfile },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT,  DbType::NEED_DATA, &DbValidator::directory }}},
        {"easy-cluster",         easycluster,          &par.easyclusterworkflow,  COMMAND_HIDDEN,
        ...
        {"search",               search,               &par.searchworkflow,       COMMAND_MAIN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"linsearch",               linsearch,               &par.linsearchworkflow,       COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"map",                  map,                  &par.mapworkflow,          COMMAND_HIDDEN,
                ...
                CITATION_PLASS|CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"cluster",              clusteringworkflow,   &par.clusterworkflow,      COMMAND_HIDDEN,
                ...
                CITATION_LINCLUST|CITATION_MMSEQS1|CITATION_MMSEQS2, {{"sequenceDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA,  &DbValidator::sequenceDb },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"linclust",          linclust,          &par.linclustworkflow,           COMMAND_HIDDEN,
        ...
        {"createindex",          createindex,          &par.createindex,          COMMAND_HIDDEN,
                "Precompute index table of sequence DB for faster searches",
                "Precomputes an index table for the sequence DB. Handing over the precomputed index table as input to mmseqs search or mmseqs prefilter eliminates the computational overhead of building the index table on the fly.",
                "Martin Steinegger <martin.steinegger@mpibpc.mpg.de>",
                "<i:sequenceDB> <tmpDir>",
                CITATION_SERVER | CITATION_MMSEQS2,{{"sequenceDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA|DbType::NEED_HEADER, &DbValidator::sequenceDb },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"createlinindex",          createlinindex,          &par.createlinindex,          COMMAND_HIDDEN,
        ...
        {"enrich",                enrich,              &par.enrichworkflow,       COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb  },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"rbh",                  rbh,                  &par.searchworkflow,       COMMAND_HIDDEN,
...
// Utility tools for format conversions
        {"createtsv",            createtsv,            &par.createtsv,            COMMAND_HIDDEN,
        ...
        {"convertalis",          convertalignments,    &par.convertalignments,    COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA|DbType::NEED_HEADER, &DbValidator::sequenceDb },
                                          ...
                                          {"alignmentFile", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile}}},
        {"convertprofiledb",     convertprofiledb,     &par.convertprofiledb,     COMMAND_HIDDEN,
        ...
        {"result2flat",          result2flat,          &par.result2flat,          COMMAND_HIDDEN,
...
// Taxonomy
        {"taxonomy",             taxonomy,             &par.taxonomy,             COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"easy-taxonomy",             easytaxonomy,             &par.easytaxonomy,             COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"createtaxdb",          createtaxdb,          &par.createtaxdb,         COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"sequenceDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"addtaxonomy",          addtaxonomy,          &par.addtaxonomy, COMMAND_HIDDEN,
        ...
        {"taxonomyreport",        taxonomyreport,  &par.threadsandcompression,   COMMAND_HIDDEN,
        ...
        {"filtertaxdb",          filtertaxdb,          &par.filtertaxdb,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"targetDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA|DbType::NEED_TAXONOMY, &DbValidator::taxSequenceDb },
                                          ...
                                          {"taxDB",   DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::taxResult }}},
// multi hit search
        {"multihitdb",           multihitdb,           &par.multihitdb,           COMMAND_HIDDEN,
        ...
        {"resultsbyset",         resultsbyset,         &par.summerizeresultsbyset,COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2,{{"",DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, NULL}}},
        {"mergeresultsbyset",    mergeresultsbyset,    &par.threadsandcompression,COMMAND_HIDDEN,
...
// Utility tools for clustering
        {"clusterupdate",        clusterupdate,        &par.clusterUpdate,        COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2|CITATION_MMSEQS1,{{"oldSequenceDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA|DbType::NEED_HEADER|DbType::NEED_LOOKUP, &DbValidator::sequenceDb },
                                          ...
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory}}},
        {"createseqfiledb",      createseqfiledb,      &par.createseqfiledb,      COMMAND_HIDDEN,
        ...
        {"mergeclusters",        mergeclusters,        &par.threadsandcompression,COMMAND_HIDDEN,
...
// Expert tools (for advanced users)
        ...
        {"ungappedprefilter",    ungappedprefilter,    &par.ungappedprefilter,    COMMAND_HIDDEN,
        ...
        {"align",                align,                &par.align,                COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"alignall",             alignall,             &par.align,                COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"transitivealign",             transitivealign,             &par.align,                COMMAND_HIDDEN,
        ...
        {"kmermatcher",          kmermatcher,          &par.kmermatcher,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2,{{"sequenceDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"prefilterDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::prefilterDb }}},
        {"kmersearch",          kmersearch,          &par.kmersearch,          COMMAND_HIDDEN,
        ...
        {"clusthash",            clusthash,            &par.clusthash,            COMMAND_HIDDEN,
...
// Utility tools to manipulate DBs
        {"compress",             compress,             &par.onlythreads,          COMMAND_HIDDEN,
        ...
        {"decompress",           decompress,           &par.onlythreads,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"DB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::allDb },
                                          {"DB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::allDb }}},
        {"apply",                apply,                &par.threadsandcompression,COMMAND_HIDDEN,
        ...
        {"extractframes",        extractframes,        &par.extractframes,        COMMAND_HIDDEN,
        ...
        {"orftocontig",          orftocontig,          &par.orftocontig,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2,{{"",DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, NULL}}},
        {"reverseseq",          reverseseq,          &par.reverseseq,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"sequenceDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::sequenceDb }}},
        {"touchdb",              touchdb,              &par.onlythreads,          COMMAND_HIDDEN,
                "Memory map database",
        ...
        {"translateaa",          translateaa,          &par.threadsandcompression,COMMAND_HIDDEN,
        ...
        {"splitdb",              splitdb,              &par.splitdb,              COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2,{{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"resultDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::resultDb }}},
        {"splitsequence",       splitsequence,         &par.splitsequence,        COMMAND_HIDDEN,
        ...
        {"createsubdb",          createsubdb,          &par.onlyverbosity,        COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"subsetFile", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::allDbAndFlat },
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::allDb },
                                          {"resultDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::allDb }}},
        {"view",          view,          &par.view,        COMMAND_HIDDEN,
        ...
        {"mvdb",          mvdb,          &par.onlyverbosity,        COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"DB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::allDb },
                                          {"DB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::allDb }}},
        {"result2profile",       result2profile,       &par.result2profile,       COMMAND_HIDDEN,
        ...
        {"result2pp",            result2pp,            &par.result2pp,            COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2,{{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::profileDb },
                                          ...
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"profileDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::profileDb }}},
        {"result2rbh",           result2rbh,           &par.threadsandcompression,COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"resultDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::resultDb }}},
        {"result2msa",           result2msa,           &par.result2msa,           COMMAND_HIDDEN,
        ...
        {"convertmsa",           convertmsa,           &par.convertmsa,           COMMAND_HIDDEN,
                ...
                CITATION_SERVER |CITATION_MMSEQS2, {{"msaFile[.gz]", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                          {"msaDB",DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::msaDb }}},
        {"msa2profile",          msa2profile,          &par.msa2profile,          COMMAND_HIDDEN,
                ...
                CITATION_SERVER |CITATION_MMSEQS2, {{"msaDB",DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::msaDb },
                                          {"profileDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::profileDb }}},
        {"profile2pssm",         profile2pssm,         &par.profile2pssm,         COMMAND_HIDDEN,
        ...
        {"profile2cs",         profile2cs,             &par.profile2cs,           COMMAND_HIDDEN,
        ...
        {"offsetalignment",      offsetalignment,      &par.offsetalignment,      COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"alnDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::alignmentDb },
                                          {"alnDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"proteinaln2nucl",      proteinaln2nucl,      &par.threadsandcompression,COMMAND_HIDDEN,
...
// Special-purpose utilities
        {"rescorediagonal",     rescorediagonal,       &par.rescorediagonal,      COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"alignbykmer",         alignbykmer,           &par.alignbykmer,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"diffseqdbs",           diffseqdbs,           &par.diff,                 COMMAND_HIDDEN,
        ...
        {"sortresult",           sortresult,           &par.sortresult,           COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"resultDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::resultDb }}},
        {"summarizealis",      summarizealis,      &par.threadsandcompression,      COMMAND_HIDDEN,
        ...
        {"summarizeresult",      summarizeresult,      &par.summarizeresult,      COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2|CITATION_UNICLUST, {{"alignmentDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::alignmentDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"summarizetabs",        summarizetabs,        &par.summarizetabs,        COMMAND_HIDDEN,
        ...
        {"gff2db",               gff2db,               &par.gff2ffindex,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"gff3File", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                          ...
                                          {"sequenceDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::sequenceDb }}},
        {"masksequence",            masksequence,            &par.threadsandcompression,          COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          {"sequenceDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::sequenceDb }}},
        {"maskbygff",            maskbygff,            &par.gff2ffindex,          COMMAND_HIDDEN,
        ...
        {"prefixid",             prefixid,             &par.prefixid,             COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"DB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::allDb },
                                          {"DB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA,  &DbValidator::allDb }}},
        {"suffixid",             suffixid,             &par.prefixid,             COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"DB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA,  &DbValidator::allDb },
                                          {"DB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA,  &DbValidator::allDb }}},
        {"convertkb",            convertkb,            &par.convertkb,            COMMAND_HIDDEN,
        ...
        {"summarizeheaders",     summarizeheaders,     &par.summarizeheaders,     COMMAND_HIDDEN,
        ...
        {"extractalignedregion", extractalignedregion, &par.extractalignedregion, COMMAND_HIDDEN,
        ...
        {"extractdomains",       extractdomains,       &par.extractdomains,       COMMAND_HIDDEN,
        ...
        {"convertca3m",          convertca3m,          &par.threadsandcompression,COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"ca3mDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::ca3mDb },
                                          {"resultDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::resultDb }}},
        {"expandaln",           expandaln,             &par.expandaln,            COMMAND_HIDDEN,
                ...
                CITATION_MMSEQS2, {{"queryDB",  DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"resultDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::resultDb },
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb }}},
        {"countkmer",           countkmer,             &par.countkmer,            COMMAND_HIDDEN,
